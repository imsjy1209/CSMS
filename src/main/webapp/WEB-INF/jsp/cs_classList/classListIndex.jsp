...
<body>
	...
	<div class="container">
		...
		<table class="table table-striped mt-5 ">
			...
			<tbody>
				<c:forEach items="${classListList}" var="item">
					<tr>
						...
						<td>${item.course.courseCategory}</td>
						<td>${item.course.courseSubject}${item.course.courseGrade}${item.course.courseClass}年級</td>
						<td>${item.course.courseMember}</td>
						<td><fmt:formatDate pattern="yyyy-MM-dd" value="${item.course.startDate}"/></td>
						<td><fmt:formatDate pattern="yyyy-MM-dd" value="${item.course.endDate}"/></td>
						<td class=" test1">
